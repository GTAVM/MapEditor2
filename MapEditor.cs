...
namespace MapEditor
{
    public class MapEditor : Script
    {
        ...
        private static void NewMethod(Entity ent, UIMenuListItem item, int index)
        {
            WeaponHash weaponHash = (WeaponHash)Enum.Parse(typeof(WeaponHash), item.IndexToItem(index).ToString());
            PropStreamer.ActiveWeapons[ent.Handle] = weaponHash;
        }
        ...
        private void RedrawObjectInfoMenu(Marker ent, bool refreshIndex)
		{
			...
			{
				MarkerType hash;
				Enum.TryParse(ite.IndexToItem(index).ToString(), out hash);
				ent.Type = hash;
			};
